...
CMD ["bash", "-c", "set -euo pipefail \
      ...
      && if [[ \"x${JET_MEMBER_ADDRESS}\" != \"x\" ]]; then export CLI_ARGS=\"${CLI_ARGS} -c ${MC_INSTALL_DIR}/hazelcast-client.xml\"; sed -i \"s/127.0.0.1/${JET_MEMBER_ADDRESS}/g;\" ${MC_INSTALL_DIR}/hazelcast-client.xml; fi \
      && echo \"########################################\" \
